...
use solver_common::error::StateError;
    ...
    async fn process_event(&mut self, event: Event) -> Vec<Action> {
        match event {
            Event::NewLimitOrderIntent(limit_order_intent) => {
                ...
                info!(?limit_order_intent, "Quoting the limit order intent");
                match self
                    .intent_quoter
                    .quote_intent(limit_order_intent.clone())
                    .await
                {
                    Ok(quoted_intent) => {
                    ...
                    Err(e) => {
                        error!(
                            ?limit_order_intent,
                            ?e,
                            "Failed to quote the limit order intent"
                        );
                    }
                }
            }
            Event::IntentQuoted(quoted_intent) => {
                ...
                self.state_manager.lock().await.update_intent_state(
                    quoted_intent.clone().limit_order_intent.intent_id,
                    |intent| intent.quoted_intent = Some(quoted_intent.clone()),
                );
                return vec![Action::LockTokensOnSourceChain(
                    quoted_intent.limit_order_intent,
                )];
            }
            Event::TokensLockedOnSourceChain(locked_tokens_intent) => {
                ...
                let limit_order_intent = &locked_tokens_intent.limit_order_intent;
