...
pub trait LockIntentTokensHandler {
    ...
    ) -> Result<LockIntentTokensHandlerResult>;
}
...
impl<H: LockIntentTokensHandler + Sync + Send> Executor<Action> for LockIntentTokensExecutor<H> {
    async fn execute(&self, action: Action) -> Result<()> {
        if let Action::LockTokensOnSourceChain(limit_order_intent) = action {
            let lock_intent_tokens_handler_result =
                self.handler.lock_tokens(limit_order_intent).await?;
