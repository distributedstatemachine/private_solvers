...
impl<H: PostLimitOrderHandler> PostLimitOrderExecutor<H> {
    pub fn new(handler: H) -> (Self, Box<dyn Collector<Event>>) {
        let (confirmation_sender, confirmation_receiver) = channel::<PostLimitOrderHandlerResult>(512);
        let fill_action_confirmation_collector =
            ...
            let fill_action_confirmation_collector: Box<dyn Collector<Event>> = Box::new(
                CollectorMap::new(fill_action_confirmation_collector, |handler_result: PostLimitOrderHandlerResult| {
...
impl<H: PostLimitOrderHandler + Sync + Send> Executor<Action> for PostLimitOrderExecutor<H> {
    async fn execute(&self, action: Action) -> Result<()> {
        if let Action::PostLimitOderIntent(limit_order_intent) = action {
            let post_limit_order_handler_result =
                self.handler.post_limit_order(limit_order_intent).await?;
